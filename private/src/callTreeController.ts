...
/*
		Classes utilisées dans ce script
*/
module CallTree {
	...
	class Noeud {
		...
		public parents: Array<LienNoeud>; // string => Noeud
		public children: Array<LienNoeud>; // string => Noeud
		public hidden: boolean;  // si le noeud est caché
		public grouped: boolean; // si le noeud a des fils cachés
	...
	/*
			class Lien
			Représente un appel entre deux applications
	*/
	interface LienBrut {
		source: string;
		target: string;
	...
	/*
		Statistiques à a propos des appels
	*/
	...
	/*
		Controlleur gérant l'arbre des Appels
		Lié à #/callTree
	*/

	export class CallTreeController {

		private color;// Fonction de coloration d3
		private force; // diagramme de force d3
		private svg; // L'élement svg d3
		private rawLinks: Array<LienBrut> = new Array();
		private links: Array<Lien> = new Array(); // Liste des liens 
		private d3link; // L'objet d3 gérant les liens
		private nodes: { [n: string]: Noeud; }; // Collection de noeuds
		private nodesValues: Array<Noeud>;
		private d3node; // l'objet d3 gerant les noeuds
		private markers; // l'objet d3 gérant les fleches
		private linkToApplications: Array<Lien> = new Array(); // les liens générant des fleches
		...
		private projector: any; // projete un élément sur un autre
		private raycaster: any; // un peu pareil, long à expliquer
		private canvas: any;
		private root3D: any;
		...
		private projectionPlan: any; // projection des coordonnées de la souris pour le déplacement
		...
		private particleMap: any; // THREE Image Texture
		...
		// place une lumière blanche Spot d'intensitée modérée en direction du centre
		...
		/*
			...
			la boucle de rendu doit être la plus légère possible:
				- on parallèlise immédiatement ce qu'on peut
				- on utilise des while avec length--
				- ou des for avec length *sauvegardé* décroissant

				- les tâches ne nécéssitant pas de calcul à chaque fois sont calculés
					peridoquement grâce à un modulo

				- pas de switch
				- pas de try catch
				- pas de 'new'
				- pas d'accès à la chaîne de prototypes
				- le moins de if possible
				- le moins de boucles possible

				- le plus critique: pas de sortie console! (console.log|warn|err)
		...
		*/
		...
		/*
			Affiche/ Cache le panneau de la liste des Noeuds
		*/
