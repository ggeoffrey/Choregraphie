...
(function (Server) {
    var Database = (function () {
        ...
        Database.prototype.getCoridors = function (callback) {
...
(function (Main) {
    ...
    var ConfigController = (function () {
        ...
        ConfigController.prototype.init = function () {
            ...
            window.Database.getCoridors(function (data) {
    ...
    var StatistiqueAppelFormater = (function () {
        function StatistiqueAppelFormater() {
            ...
            this.format = function (caller, called, value) {
                ...
                _this.invalide = !!(_this.caller && _this.called && _this.service && _this.methode && _this.value);
            };
            this.toCSV = function () {
                if (_this.invalide)
                    return _this.caller.nom + ';' + _this.called.nom + ';' + _this.service + ';' + _this.methode + ';' + _this.value + '\n';
                else
                    return '';
            };
            this.invalide = false;
    ...
    var CallTreeController = (function () {
        ...
        CallTreeController.prototype.load = function (callback) {
            ...
            if (sessionStorage.getItem('callTree')) {
                ...
                this.$http.get('api/get/appels').success(function (data) {
        ...
        CallTreeController.prototype.selectNode = function (nomSelect, mouseover) {
            var _this = this;
            var nom;
            var node;
            for (nom in this.nodes) {
        ...
        CallTreeController.prototype.selectParents = function (nomSelect, depth) {
        ...
        CallTreeController.prototype.getNodeCalls = function (node) {
            ...
            while (lengthLinks--) {
                ...
                if (rawLink.source == node.nom || rawLink.target == node.nom) {
                    count += rawLink.value;
                }
            }
...
(function (CasParCas) {
    ...
    CasParCas.Statistique = Statistique;

    var CasParCasController = (function () {
        function CasParCasController($scope, $http, $routeParams, eventsController) {
            var _this = this;
            ...
            this.init = function () {
                ...
                if (listeApp) {
                    ...
                    _this.http.get('api/get/applications').success(function (data) {
                        nextApp(data);
                ...
                if (listeCouloirs) {
                    ...
                    _this.http.get('api/get/couloirs').success(function (data) {
                        nextCouloirs(data);
            ...
            this.load = function () {
                if (_this.application && _this.couloir) {
                    ...
                    _this.http.get("api/get/totalErreurs?application=" + _this.application + "&couloir=" + _this.couloir).success(function (data) {
                        if (data.length > 0) {
                    ...
                    window.startLoader();
                    _this.http.get("api/get/trend?app=" + _this.application + "&couloir=" + _this.couloir).success(function (data) {
                        var type;
                        for (type in data) {
            ...
            this.buildBrush = function (mustRefresh) {
                ...
                var eventsEnter = eventsContainer.enter().append('rect').attr('x', function (event) {
                    return _this.scalerBrushX(event.startTime);
                ...
                if (!skipRendering && data.length > 0) {
                    ...
                    try  {
                        ...
                        var eventsEnter = eventsContainer.enter().append('line').attr('x1', function (event) {
                            return scalerX(event.startTime);
        ...
        return CasParCasController;
    })();
    CasParCas.CasParCasController = CasParCasController;
})(CasParCas || (CasParCas = {}));

(function () {
    window.ccolControllers.controller('casParCasController', [
        '$scope', '$http', '$routeParams', '$window', function ($scope, $http, $routeParams, $window) {
            ...
            $scope.vm = new CasParCas.CasParCasController($scope, $http, $routeParams, eventsController);
        }]);
})();
var Events;
(function (Events) {
    var EventsController = (function () {
        ...
        EventsController.prototype.setEventSeen = function (event) {
            window.startLoader();
            this.http.get('api/set/events?action=seen&target=' + event.id + '&value=' + event.seen).success(function (data) {
                console.log(data);
